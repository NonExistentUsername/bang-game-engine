...
class TestEngine:
    def test_damage(self, engine: Engine):
        ...
        user_bullets = engine.get_player(0).bullets

        engine.damage_player(0, 1)

        assert (
            engine.get_player(0).bullets == user_bullets - 1
    ...
    def test_damage_multiple(self, engine: Engine):
        ...
        assert (
            engine.get_player(0).bullets == user_bullets - 3
    ...
    def test_heal(self, engine: Engine):
        ...
        assert (
            engine.get_player(0).bullets == user_bullets - 2
        ), "Player bullets should be decreased by 2"

    def test_draw_cards(self, engine: Engine):
        previous_user_hand_size = len(engine.get_player(0).hand)

        engine.draw_cards(0, 1)

        assert len(engine.get_player(0).hand) == previous_user_hand_size + 1
